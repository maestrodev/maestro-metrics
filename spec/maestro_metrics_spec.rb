...
describe Maestro::Metrics do
  ...
  before(:all) do
    Maestro::Metrics.configure( { :mongo_host => '192.168.56.10', :statsd_host => '192.168.56.10' } )
  end

  before(:each) do
    Maestro::Metrics.unmock!
  end
  ...
  context 'Stats logging' do
    ...
    it 'should time the execution of a block of code' do
      ...
      Maestro::Metrics.time(@metric,1, &proc)
      ran.should be_true

    end

    it 'should do nothing when using a mock' do
      Maestro::Metrics.mock!
      value = 8
  ...
  context 'raw metrics logging' do
    ...
    it 'should log a hash containing raw metrics' do
      ...
      Maestro::Metrics::Real.instance.stub(:mongo_collection => @collection)

      doc = { :name => 'testdoc' }
      @collection.stub(:insert => 1)
      @collection.should_receive(:insert).with(doc)

      Maestro::Metrics.log("test", doc).should eq 1
    ...
    #    }
    #
    #  Maestro::Metrics.log('runs', doc)
    #  results = Maestro::Metrics.aggregate('runs',
